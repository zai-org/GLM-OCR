...
Third solution: (by Richard Stanley) Write $ x= $ $(x_{1},\dots,x_{n}) $ and $ \nabla=\left(\frac{\partial}{\partial x_{1}},\dots,\frac{\partial}{\partial x_{n}}\right). $ Suppose that $ P(x)=Q(x)\left(x_{1}^{2}+\cdots+x_{n}^{2}\right). $ Then
...
$$
f (S) = \sum_ {T \supset S} (- 1) ^ {| T | - | S |} \sum_ {U \supseteq T} f (U).
$$
